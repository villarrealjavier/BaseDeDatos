...
OR UPPER(CIUDAD_EMPRESA) LIKE 'MÁLAGA'
...
FROM DESTINO
WHERE CIUDAD_DESTINO LIKE '%b%' 
OR CIUDAD_DESTINO LIKE '%B%';
...
FROM TRASLADO
WHERE EXTRACT(YEAR FROM FECHA_ENVIO)=1994 AND EXTRACT(MONTH FROM FECHA_ENVIO)=12;
...
SELECT FECHA FROM RESIDUO_EMPRESA;
...
SELECT DISTINCT COUNT(RESIDUO_CONSTITUYENTE.COD_CONSTITUYENTE),RESIDUO_EMPRESA.NIF_EMPRESA
FROM RESIDUO_CONSTITUYENTE,RESIDUO,RESIDUO_EMPRESA
WHERE RESIDUO_CONSTITUYENTE.COD_RESIDUO = RESIDUO.COD_RESIDUO 
AND RESIDUO.COD_RESIDUO=RESIDUO_EMPRESA.COD_RESIDUO
...
FROM EMPRESAPRODUCTORA, RESIDUO_EMPRESA
WHERE EMPRESAPRODUCTORA.NIF_EMPRESA = RESIDUO_EMPRESA.NIF_EMPRESA 
AND UPPER(EMPRESAPRODUCTORA.OD_EMPRESA) LIKE '%METALES%';

SELECT OD_EMPRESA FROM EMPRESAPRODUCTORA;
...
SELECT COUNT(TRASLADO.COD_DESTINO), EMPRESATRANSPORTISTA.CIUDAD_EMPTRANSPORTE, DESTINO.CIUDAD_DESTINO 
FROM EMPRESATRANSPORTISTA, DESTINO, TRASLADO
WHERE EMPRESATRANSPORTISTA.NIF_EMPTRANSPORTE = TRASLADO.NIF_EMPTRANSPORTE AND TRASLADO.COD_DESTINO = DESTINO.COD_DESTINO
GROUP BY EMPRESATRANSPORTISTA.CIUDAD_EMPTRANSPORTE, DESTINO.CIUDAD_DESTINO ;
...
SELECT OD_RESIDUO FROM RESIDUO;

SELECT EMPRESATRANSPORTISTA.NOMBRE_EMPTRANSPORTE, TRASLADO.FECHA_ENVIO 
FROM EMPRESATRANSPORTISTA, TRASLADO, EMPRESAPRODUCTORA,RESIDUO,RESIDUO_EMPRESA
...
AND RESIDUO_EMPRESA.COD_RESIDUO = RESIDUO.COD_RESIDUO 
AND (UPPER(EMPRESAPRODUCTORA.CIUDAD_EMPRESA)='MALAGA' 
OR UPPER(EMPRESAPRODUCTORA.CIUDAD_EMPRESA)='MÁLAGA' 
OR UPPER(EMPRESAPRODUCTORA.CIUDAD_EMPRESA)='HUELVA') 
...
SELECT NOMBRE_EMPRESA FROM RESIDUO,RESIDUO_EMPRESA,EMPRESAPRODUCTORA WHERE RESIDUO_EMPRESA.NIF_EMPRESA = EMPRESAPRODUCTORA.NIF_EMPRESA  
AND RESIDUO.COD_RESIDUO =RESIDUO_EMPRESA.COD_RESIDUO 
...
WHERE RESIDUO_EMPRESA.NIF_EMPRESA = EMPRESAPRODUCTORA.NIF_EMPRESA  
AND RESIDUO.COD_RESIDUO =RESIDUO_EMPRESA.COD_RESIDUO 
AND UPPER(EMPRESAPRODUCTORA.NOMBRE_EMPRESA) LIKE '%C%'
